...
All cards try to follow a **"base effect + conditional bonus"** pattern:
...
**"Go Again" conundrum:**
- Sometimes a negative, as passing a turn can force an opponent into a bad play. The last thing you want as a player is your opponent taking multiple actions that you cannot respond to. 
- This means playing a low impact card that doesn't really affect the board, but passes prority back to your opponent can put them in a worse place. 
- However, also keeps initiative = significant tactical advantage.
- Needs to be reviewed card by card, no hard and fast rule.
...
**6. Strafe Run** (NEW CARD - SIMPLIFIED IMPLEMENTATION)
- **Type:** Ordnance
- **Cost:** 3 energy (reduced from 4 due to simplified targeting)
- **Max in Deck:** 4
- **Effect:** Deal 1 damage to up to 3 enemy drones in target lane (front to back).
- **Targeting:** Enemy lane
- **Current Implementation:** Lane-based with maxTargets=3
- **Original Design:** Multi-lane (1 drone per lane across 3 lanes) - requires multi-target UI
- **Visual:** Placeholder LASER_BLAST (future: fighter craft flyby)
- **Design notes:** Simplified for MVP. Lane-wide pressure with limited targets. Original cross-lane design documented for future UI enhancement. Still provides good value by damaging/destroying multiple drones in concentrated lane.
...
- [X] Add `marked` boolean to drone game state
- [X] This means that drones can only be marked once. 
- [X] Implement mark application function
- [X] Implement mark removal (the mark is removed from the drone when it is destroyed - cards in the future may also be able to remove marks from targets)
- [X] Add UI visual indicator (target lock icon on marked drones)
- [X] Update targeting validation (cards that require marked targets)
- [X] Add a new Ship Cards that allows for the makring of drones. Base this off the existing Bridge card, but it has a different Ship abiility that costs 2 energy and marks any valid target enemy drone (i.e., it is not already marked). The ability can be called Target Lock. 

### Phase 2: Game Engine - New Keywords
- [X] **OVERFLOW:** Implement damage overflow calculation
- [X] **OVERFLOW:** Route excess damage to ship section in lane
- [X] **OVERFLOW:** Visual effect continuation to ship
- [X] **SPLASH:** Define adjacency rules (all-in-lane vs positional)
- [X] **SPLASH:** Implement splash damage to adjacent drones
- [X] **SPLASH:** Visual shockwave effect

### Phase 3: Card Data Updates
- [X] Update Laser Blast: Cost 3→2, add marked conditional *(completed 2025-10-29)*
- [X] Update Target Lock: Cost 6→3, change to marked-only targeting *(completed 2025-10-29)*
- [X] Keep Piercing Shot as-is (verify maxInDeck, cost)
- [X] Keep Shrieker Missiles as-is
- [X] Keep Sidewinder Missiles as-is
- [X] Keep Nuke as-is
- [X] Move System Sabotage from Ordnance to Tactics category *(completed 2025-10-29)*

### Phase 3.5: Game Logic Enhancements (Completed 2025-10-29)
- [X] **markedBonus Support:** Damage effects can now have markedBonus field for bonus damage to marked targets (Laser Blast)
- [X] **DAMAGE_SCALING Effect Type:** New effect type that calculates damage based on game state
  - Implemented source: READY_DRONES_IN_LANE (Overwhelming Force - CARD035)
- [X] **ALL_MARKED Targeting Type:** Auto-targets all marked enemy drones (Purge Protocol - CARD036)
- [X] **MULTI_DRONE Targeting Type:** Returns all valid drones for multi-target selection
- [X] **maxTargets Support:** FILTERED damage effects can now limit number of drones affected (Strafe Run - CARD034)
- [X] **Build Verification:** All changes compile successfully

### Phase 4: New Card Implementation
- [X] Create Finishing Volley (2 cost, destroy exhausted) *(completed 2025-10-29 - CARD033)*
- [X] Create Strafe Run - **SIMPLIFIED:** Now 3 cost, lane-based (up to 3 drones in target lane). Original multi-lane design deferred pending multi-target UI. *(completed 2025-10-29 - CARD034)*
- [X] Create Barrage (4 cost, splash, conditional) *(previously completed - CARD032)*
- [X] Create Railgun Strike (5 cost, overflow, marked bonus) *(previously completed - CARD031)*
- [X] Create Overwhelming Force (5 cost → 2 cost, lane-based scaling) *(completed 2025-10-29 - CARD035)*
- [X] Create Purge Protocol (7 cost, mass marked destroy) *(completed 2025-10-29 - CARD036)*
...
- [X] **Barrage:** Artillery shell + shockwave splash
- [ ] **Shrieker:** Fast missile swarm (white/silver)
- [X] **Railgun Strike:** Massive beam with overflow continuation
...
- [X] Update Bridge card to have marking ability
- [X] Define marking cost (2 energy)
- [X] Define marking scope (single target vs lane-wide)
- [X] Implement marking action in game engine
- [X] UI for marking action selection

### Phase 7: Marking Drone Cards (Optional - Phase 1.5)
- [X] Design 2-3 drones with marking abilities
- [X] Decide on marking trigger (on attack, on deploy, active ability)
- [X] Implement drone marking logic
- [X] Balance marking frequency vs payoff strength
...
### Completed (2025-10-29 Session):
**Card Updates:**
- Laser Blast (CARD001): Cost reduced 3→2, markedBonus damage added
- Target Lock (CARD009): Cost reduced 6→3, now requires marked targets only
- System Sabotage (CARD022): Type changed Ordnance→Tactic

**New Cards Created:**
- Finishing Volley (CARD033): 2 cost, destroy exhausted drones
- Strafe Run (CARD034): 3 cost, lane-based multi-target (simplified from original design)
- Overwhelming Force (CARD035): 2 cost, scaling damage based on ready friendly drones in lane
- Purge Protocol (CARD036): 7 cost, destroy all marked enemy drones

**Game Logic Enhancements:**
- markedBonus damage modifier
- DAMAGE_SCALING effect type with READY_DRONES_IN_LANE source
- ALL_MARKED targeting type
- MULTI_DRONE targeting type
- maxTargets limit for FILTERED effects

### Previously Completed:
- Barrage (CARD032): SPLASH_DAMAGE with conditional bonus
- Railgun Strike (CARD031): OVERFLOW_DAMAGE with marked bonus
- Marking system (ship abilities, status effects, targeting validation)
- OVERFLOW and SPLASH keyword mechanics

### Status: 12/12 Ordnance Cards Implemented
All card definitions complete. Visual effects using placeholders (to be enhanced later).

### Known Simplifications:
- **Strafe Run**: Implemented as lane-based (up to 3 drones in one lane) instead of original multi-lane design. Full multi-target UI support deferred for future enhancement.
