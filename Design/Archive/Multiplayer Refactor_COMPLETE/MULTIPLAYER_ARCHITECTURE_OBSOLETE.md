...
**[FIXED] D2. `processNetworkAction()` is never called**
Removed in Phase 9. Was never called — guest actions arrive via `processGuestAction()`.

**[FIXED] D3. Orphaned `state_sync_requested` handler**
Removed in Phase 9. Sync now goes through `requestFullSync()`/`sendFullSyncResponse()` in RemoteGameServer.

**[FIXED] D4. `render_complete` event emitted but never consumed**
Removed in Phase 9. All emit sites and the useGameState filter deleted.
