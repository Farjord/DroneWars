...
| 62 | DrawEffectProcessor.js, GainEnergyEffectProcessor.js, MovementEffectProcessor.js | `pairSet` not propagated across cascade boundaries — each effect processor creates a fresh TriggerProcessor with empty pairSet, allowing unbounded recursive trigger chains that only stop when deck empties or stack overflows. Must pass pairSet through EffectRouter. | Trigger architecture audit | 2026-03-01 | Critical |
| 63 | TriggerProcessor.js | `chainDepth` tracked but never enforced — no `MAX_CHAIN_DEPTH` constant or check exists. Add safety limit as guardrail against infinite cascades. | Trigger architecture audit | 2026-03-01 | Critical |
| 64 | DrawEffectProcessor.js | `sourceId` always `'system'` for card draws — no `triggeringDrone` passed to `fireTrigger()`. Prevents correct pair-key generation for cascades (should be reactor drone ID). Same issue in GainEnergyEffectProcessor and MovementEffectProcessor. | Trigger architecture audit | 2026-03-01 | Important |
| 65 | Trigger system (7 files) | Compute-all-then-animate architecture requires 7 timing workarounds (STATE_SNAPSHOT, TRIGGER_CHAIN_PAUSE, preCascadePlayerStates, additive/destructive ordering, post-process card removal, stateBeforeTriggers, breathing room pauses). Refactor to structured action list where each step carries its own stateAfter snapshot. See `Design/TRIGGER_SYSTEM_ARCHITECTURE_AUDIT.md`. | Trigger architecture audit | 2026-03-01 | Medium |
