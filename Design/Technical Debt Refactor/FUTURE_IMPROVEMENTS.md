...
| ~~49~~ | ~~useResolvers.js:148~~ | ~~`resolveShipAbility` lacks try/catch around `processActionWithGuestRouting`~~ — moved to Resolved | | |
| ~~50~~ | ~~useMultiplayerSync.js:249 (now useWaitingForOpponent.js)~~ | ~~`render_complete` event fires on every `gameState` change~~ — moved to Resolved | | |
...
| 84 | useGameLifecycle.js, useWaitingForOpponent.js | Hooks accessing GameStateManager internals directly (e.g., `gameStateManager._updateContext`, direct property reads) — consider adding accessor methods to GSM to reduce coupling and protect internal state shape. | Post-refactor review (D18) | 2026-03-09 | Low |
...
| 86 | ActionProcessor.js / processQueue | When any action processor throws, `action_completed` is never emitted — the game permanently freezes with no error visible to the user. `processQueue` should wrap each action in try/catch, emit `action_completed` with an error flag on failure, and log the error. This would prevent silent freezes from any unhandled action error, not just the SINGLE_MOVE case fixed in this commit. | AI Maneuver+ crash fix | 2026-03-10 | Medium |
| 87 | EffectChainProcessor.movementBlocked.test.js:162-175 | Third test checks `result.stateBeforeTriggers` and `result.actionSteps` but `processEffectChain` never returns these properties — assertions are dead code (always skipped by `if` guards). Clean up or remove. | Snapshot isolation fix code review | 2026-03-10 | Low |
| 88 | [RESOLVED] MovementEffectProcessor.js, EffectRouter.js | MULTI_MOVE effect type fully removed. Reposition now uses 3 SINGLE_MOVE effects with optional chain support. `executeMultiMove` and `executeMultiMoveForResult` deleted. MULTI_MOVE removed from EffectRouter registry. | MULTI_MOVE decomposition | 2026-03-14 | Resolved |
...
| 98 | public/Factions/*.png | Faction logos unoptimized — Neutral_1.png is 2.3MB. Convert all to WebP or compressed PNG for faster loading. | Faction logo implementation | 2026-03-20 | Low |
| 99 | deckFilterUtils.js | At 691 lines — three parallel filter/chipGen/count/default triads (cards, drones, enhancer) share structural repetition. Extract shared chip generation helper or split enhancer triad into own file. | Filter panel overhaul code review | 2026-03-20 | Low |
| 100 | useEffectChain.js — filterFullDestinations | Duplicates the real-count + ghost-count ceiling arithmetic that also lives in `isLaneFull`/`getLaneCapacity`. If `MAX_DRONES_PER_LANE` semantics ever become nuanced (e.g., drone-type caps), only one place will be updated. Refactor to delegate to `getLaneCapacity`. | Lane capacity bug fix code review | 2026-04-13 | Low |
| 101 | useEffectChain.test.js — MockPositionTracker | Hook test mock does not pre-populate `dronePositions` from `playerStates` (real `PositionTracker` does). Tests pass because no current test calls `getDronePosition` before a `recordMove`, but future tests relying on initial board positions will silently get `null`. Align mock with real behaviour. | Lane capacity bug fix code review | 2026-04-13 | Low |
| 102 | ShipSectionCompact.jsx — isAffectedSection | `animate-pulse` on the affected-section overlay was intentionally excluded from the `.valid-target` unification. It is a passive collateral-damage indicator for NONE-targeting cards (Crossfire, Encirclement), not a targeting signal. If the visual language evolves, consider a dedicated `.affected-section` CSS class for clarity. | valid-target unification | 2026-04-14 | Low |
| 103 | FACTION_COLORS import paths | Components added before the `valid-target` unification import `FACTION_COLORS` from `ShipSectionLayers.jsx`; components added during or after import directly from `src/utils/factionColors.js`. Standardise all imports to the utility file to remove the re-export dependency. | valid-target unification | 2026-04-14 | Low |
| 104 | CardRevealOverlay.jsx, DroneActionAnnouncementOverlay.jsx | `transition-all duration-300` Tailwind class hardcodes 300ms — won't follow if `ACTION_ANNOUNCEMENT_FADE_MS` changes. Both files have a cross-reference comment but structural protection requires an inline `style` or CSS variable. Fix both together. | drone action announcements review | 2026-04-16 | Low |
| 105 | CardRevealOverlay.jsx | Latent timer-cleanup bug: inner `cleanupTimer` is created inside the `displayTimer` callback and its `return () => clearTimeout(cleanupTimer)` is unreachable by React. On unmount during the fade window, the cleanup timer fires on a dead component. `DroneActionAnnouncementOverlay` uses the correct `let cleanupTimer` + single returned cleanup. Align `CardRevealOverlay` to the same pattern. | drone action announcements review | 2026-04-16 | Low |
| 104 | useResolvers.js:383 | `handleConfirmCardPlay` — when `chainSelections` is present, top-level `insertionIndex` from `cardConfirmation` is not forwarded to `submitAction`. Currently harmless (movement cards always use chainSelections and embed insertionIndex per-selection), but would silently drop a top-level insertionIndex if a non-chained movement card were ever added. | movement insertionIndex fix | 2026-04-14 | Low |
| 105 | useEffectChain.js — `confirmChainTarget` / click-confirm path for compound effects | `confirmChainTarget` calls `selectChainTarget(pendingTarget, pendingLane)` without an `insertionIndex`. For auto-resolved compound effect destinations (e.g. Forced Repositioning effect[1]), the click-confirm path therefore always appends the drone. Currently acceptable — no UI exists to pick a position during `subPhase: 'target'`. If a future UX adds position selection to the click-confirm flow, store `pendingInsertionIndex` in chainState alongside `pendingTarget`/`pendingLane` and forward it through `confirmChainTarget`. | movement insertionIndex fix | 2026-04-15 | Low |
| 106 | announcementUtils.js, AnnouncementQueue.js | Drone action items (`droneAttack`/`droneMove`) are distinguished from phase items only by absence of `phaseText`. The current merge guard uses `item.phaseText !== undefined` as a proxy for "is phase-style". A cleaner approach: add an explicit `category: 'drone' \| 'phase'` field to all announcement items in `extractAnnouncements`, and use it in `AnnouncementQueue._playNext`. This makes the distinction self-documenting and removes implicit coupling between shape and semantics. | crash fix: mixed-queue compound | 2026-04-16 | Low |
...
| 50 | useMultiplayerSync.js:249 (now useWaitingForOpponent.js) | `render_complete` event fires on every `gameState` change — excessive fire rate | 2026-03-05 | Event deleted entirely in Multiplayer Refactor Phase 9 |
...
| 49 | useResolvers.js:148 | `resolveShipAbility` lacks try/catch around `processActionWithGuestRouting` | 2026-03-13 | Dead code removed — `resolveShipAbility` client function deleted; all ship abilities route through specific action types via `handleConfirmShipAbility` |
| 97 | DronesView, CardWarningOverlay.jsx | Migrate DronesView to use ActionCardTooltipPanel, then remove CardWarningOverlay.jsx | 2026-03-20 | DronesView now uses inline ActionCardTooltipPanel with droneCardTooltipDescriptions. CardWarningOverlay deleted. Full pipeline removed from App.jsx, GameFooter, AnimationLayer. |
