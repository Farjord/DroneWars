  ...
  const resolveAbility = useCallback((ability, userDrone, targetDrone) => {
    ...
    if (effect.type === 'HEAL') {
        setPlayer1(prev => {
            ...
            // Check if the ability is lane-wide
            if (effect.scope === 'LANE') {
                const targetLaneId = targetDrone.id; // For lane targets, the target IS the lane object
                if (newDronesOnBoard[targetLaneId]) {
                    newDronesOnBoard[targetLaneId].forEach(droneInLane => {
                        ...
                        // Only heal if the drone is actually damaged
                ...
                // Original logic for single-target heals
                const baseTarget = fullDroneCollection.find(d => d.name === targetDrone.name);
                const targetLaneId = getLaneOfDrone(targetDrone.id, prev);
                if (targetLaneId) {
...
// The line before the change
            ...
            // --- [START] NEWLY ADDED CODE ---
              ...
              // --- NEW: Differentiate between lane and single-drone targets ---
              if (card.targeting?.type === 'LANE') {
                  const laneId = target.id;
                  const dronesInLane = player2.dronesOnBoard[laneId] || [];

                  if (dronesInLane.length === 0) {
                      score = 0; // No value in buffing an empty lane
                      action.logic.push('No Drones in Lane');
                  } else {
                      const currentLaneScore = calculateLaneScore(laneId, player2, player1, opponentPlacedSections, placedSections, getShipStatus, calculateEffectiveStats, calculatePotentialEffectiveStats);
                      
                      // Simulate the buff on all drones in the lane
                      const tempAiState = JSON.parse(JSON.stringify(player2));
                      ...
                      const projectedLaneScore = calculateLaneScore(laneId, tempAiState, player1, opponentPlacedSections, placedSections, getShipStatus, calculateEffectiveStats, calculatePotentialEffectiveStats);
                      const laneImpact = projectedLaneScore - currentLaneScore;
                      
                      // Score is based on impact, plus a bonus for hitting multiple drones
                  ...
                  // --- Original logic for single-drone targets ---
                  if (mod.stat === 'attack' && mod.value > 0) { // Attack Buff
                      if (target.isExhausted) {
                          score = -1;
                          action.logic.push('Invalid (Exhausted)');
                      } else {
                          // Simplified scoring for single buffs
              ...
              // Apply general modifiers if the action is considered valid
              if (score > 0) {
  ...
  const handleCardClick = (card) => {
    ...
    if (selectedCard?.instanceId === card.instanceId) {
      cancelCardSelection();
        ...
        const validTargets = getValidTargets('player1', null, card);
        // Use the correct state setter from your file
        setUpgradeDestroySelection({ card, targets: validTargets, opponentState: player2 });
        setSelectedCard(null);
        setAbilityMode(null);
        setSelectedDrone(null);
          ...
          <div className="flex flex-col items-center w-full mt-2">
            ...
            <div className="flex w-full justify-center gap-1">
              {Array.from({ length: stats.maxHull }).map((_, i) => (
                <div key={i} className={`h-4 w-4 rounded-sm ${i < stats.hull ? 'bg-green-500' : 'bg-red-500'} border border-black/50`}></div>
              ))}
            </div>
          </div>
        </div>
