...
const fullDroneCollection = [
  ...
  { name: 'Guardian Drone', class: 3, limit: 2, attack: 2, hull: 5, shields: 3, speed: 1, image: 'https://placehold.co/128x128/00f/fff?text=Guardian', abilities: [] },
    ...
    abilities: [{ 
        ...
        description: 'Deals +4 damage when attacking an enemy ship section.',
        type: 'PASSIVE',
        icon: Bomb,
  ...
  const resolveAttack = useCallback((attackDetails, isAbilityOrCard = false) => {
    ...
    let damage = abilityDamage ?? attacker.attack;
      if (finalTargetType === 'section' && !abilityDamage) {
        // Find the full drone data to check for abilities
...
setPlayer2(prev => {
        ...
        // --- NEW: AI draws cards to its hand limit ---
...
// If the AI has already passed, it cannot take any more actions. End the turn immediately.
    ...
    for (const card of playableCards) {
      if (card.targeting) {
        const targets = getValidTargets('player2', null, card);
    ...
    for (const attacker of readyAiDrones) {
      // Action: Attack enemy drones in the same lane
      ...
      // The AI ALWAYS considers attacking the ship section.
      ...
      switch (action.type) {
        ...
        case 'play_card':
          ...
          if (card.effect.type === 'DESTROY') {
            let cpuDestroyed = 0;
            if (card.effect.scope === 'SINGLE') { cpuDestroyed = target.class || 0; }
            score = (cpuDestroyed * 10) - (card.cost * 5);
        ...
        case 'attack':
          ...
          if (targetType === 'drone') {
            // Priority: Favorable Trades & High-Value Destruction
            score = (attackTarget.class * 10);
            if (attacker.class < attackTarget.class) { score += 20; }
            if (!attackTarget.isExhausted) { score += 10; }
      } else if (targetType === 'section') {
        // ---  Calculate potential damage including ship-buster bonus ---
        let potentialDamage = attacker.attack;
        const baseAttacker = fullDroneCollection.find(d => d.name === attacker.name);
        baseAttacker?.abilities?.forEach(ability => {
            if (ability.type === 'PASSIVE' && ability.effect.type === 'BONUS_DAMAGE_VS_SHIP') {
                potentialDamage += ability.effect.value;
            }
        });

        // Priority: Damaging the Player's Ship, now using potential damage
        score = (potentialDamage * 8); // Use potential damage here
        // Give a massive bonus for using the ability as intended
        if (potentialDamage > attacker.attack) {
            score += 50; 
        }
        ...
        // Prioritize attacking sections with no shields for direct hull damage.
        if (attackTarget.allocatedShields === 0) {
            score += 40;
        } 
        // Prioritize breaking through the last shields.
        else if (potentialDamage >= attackTarget.allocatedShields) { // Use potential damage here
            score += 35;
        }
        // Add a small bonus for any reasonably strong attack.
        if (potentialDamage >= 3) { // Use potential damage here
            score += 10;
        }
      }
          action.score = score;
    ...
    const isAiTurn = currentPlayer === 'player2' && !winner && !aiActionReport && !aiCardPlayReport && !pendingAttack && !playerInterceptionChoice && !mandatoryAction;
    if (!isAiTurn) return;
  ...
  }, [currentPlayer, turnPhase, passInfo, winner, handleOpponentTurn, handleOpponentAction, aiActionReport, aiCardPlayReport, pendingAttack, playerInterceptionChoice, mandatoryAction]);
 ...
 const handleSelectOpponent = (selectedAI) => {
  ...
  const handleConfirmDroneSelection = (selectedDrones) => {
   ...
   selectedDrones.forEach(drone => {
    ...
    // --- NEW: AI draws its starting hand at the same time ---
    setPlayer2(prev => {
      let newDeck = [...prev.deck];
      let newHand = []; // Start with an empty hand
      const handSize = calculateEffectiveShipStats(prev).handLimit;

      for (let i = 0; i < handSize; i++) {
        // For the initial draw, we assume the deck has enough cards.
        // No need to check for reshuffling the discard pile yet.
        if (newDeck.length > 0) {
          newHand.push(newDeck.pop());
        } else {
    ...
    const handleCloseAiCardReport = useCallback(() => {
        ...
        if (aiCardPlayReport && !aiCardPlayReport.card.effect.goAgain) {
            endTurn('player2');
             ...
             setModalContent({ title: "Opponent's Turn", text: "The AI takes another action!" });
        }
