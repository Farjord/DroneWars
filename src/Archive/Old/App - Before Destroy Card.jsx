...
// --- NEW: LOGIC TO RESOLVE A CARD ---
  const resolveCardPlay = useCallback((card, target) => {
    ...
    // --- FIX START: All modifications to player1 now happen in this single block ---
    setPlayer1(prev => {
      // Start with the state after paying the cost
      let newState = {
        ...prev,
      ...
      // Apply card effects that modify the current player's state
      if (effect.type === 'DRAW') {
        ...
        newState = { ...newState, deck: newDeck, hand: newHand, discardPile: newDiscardPile };
        ...
        const isHealingHull = effect.type === 'HEAL_HULL';
        const healAmount = effect.value;

        if (card.targeting.type === 'DRONE') {
          const targetLaneKey = Object.keys(newState.dronesOnBoard).find(lane => 
            newState.dronesOnBoard[lane].some(drone => drone.id === target.id)
          );
          ...
          const sectionToUpdate = newShipSections[target.id];
          if (sectionToUpdate) {
            if (isHealingHull) {
          ...
          const newDronesOnBoard = JSON.parse(JSON.stringify(newState.dronesOnBoard));
          const laneToUpdate = newDronesOnBoard[target.id];
          if (laneToUpdate) {
    ...
    // Effects targeting other players are handled separately
    ...
    // This block is now only for healing the opponent (player2), if such a card ever exists.
    else if ((effect.type === 'HEAL_HULL' || effect.type === 'HEAL_SHIELDS') && target.owner === 'player2') {
      const isHealingHull = effect.type === 'HEAL_HULL';
      const updater = setPlayer2; // Directly set to player2

      updater(prev => {
        // ... The same healing logic as above, but it would now correctly target player2 ...
        // This part of the code is currently unreachable as no cards heal the opponent,
        // but the structure is now correct for future expansion.
        return prev;
      });
    }
  ...
  }, [player1, player2, getLaneOfDrone, resolveAttack, endTurn, calculateEffectiveShipStats]);
