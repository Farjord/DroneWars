...
useEffect(() => {
  ...
  const handleOpponentTurn = useCallback(() => {
    ...
    // --- AI Deployment Decision Logging ---
    const deploymentContext = [];
    const availableResources = turn === 1 
      ? (player2.initialDeploymentBudget + player2.energy) 
      : (player2.deploymentBudget + player2.energy);

    player2.activeDronePool.forEach(drone => {
      const option = {
        instigator: drone.name,
        logic: [],
        score: 0, // Lower cost = higher score
      };
      const cost = drone.class;
      const currentCount = player2.deployedDroneCounts[drone.name] || 0;
      let isPossible = true;

      if (availableResources < cost) {
        option.logic.push(`Cannot afford: Cost ${cost} > Resources ${availableResources}.`);
        isPossible = false;
      }
      if (currentCount >= drone.limit) {
        option.logic.push(`At deploy limit: ${currentCount}/${drone.limit}.`);
        isPossible = false;
      }
      if (totalDrones >= effectiveStats.cpuLimit) {
        option.logic.push(`CPU limit reached: ${totalDrones}/${effectiveStats.cpuLimit}.`);
        isPossible = false;
      }

      if (isPossible) {
        option.score = 100 - cost; // Prioritize cheaper drones
        option.logic.push(`Viable. Score: ${option.score} (based on low cost).`);
      }
      deploymentContext.push(option);
    });
    ...
    const canDeploy = deployableDrones.length > 0;
    const willPass = !canDeploy;

    if (willPass) {
      addLogEntry({ player: player2.name, actionType: 'PASS', source: 'N/A', target: 'N/A', outcome: `Passed during deployment phase.` }, 'aiDeploymentPass', deploymentContext);
      setPassInfo(prev => {
      ...
      const chosenActionLog = deploymentContext.find(log => log.instigator === droneToDeploy.name);
      if (chosenActionLog) {
        chosenActionLog.isChosen = true;
        chosenActionLog.targetName = targetLane;
      }

      addLogEntry({ player: player2.name, actionType: 'DEPLOY', source: droneToDeploy.name, target: targetLane, outcome: `Deployed to ${targetLane}.` }, 'aiDeploymentDeploy', deploymentContext);
  ...
  }, [passInfo, turn, endTurn, updateAuras, endDeploymentPhase, player2, addLogEntry]);
  ...
  const AIDecisionLogModal = ({ decisionLog, onClose }) => {
    ...
    const formatTarget = (action) => {
      ...
      if (action.type === 'deploy' || !action.target) {
        return action.isChosen ? action.targetName : 'N/A';
      }
