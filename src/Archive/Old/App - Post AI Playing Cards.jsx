...
// --- NEW: AI PERSONALITY DEFINITIONS ---
const aiPersonalities = [
  {
    ...
    decklist: [
      ...
      { id: 'CARD009', quantity: 2 }, // Target Lock (single-target destroy)
...
const handleOpponentAction = useCallback(() => {
    ...
    // --- NEW: AI CARD PLAYING LOGIC ---
    // 1. Find all cards the AI can afford to play.
    const playableCards = player2.hand.filter(card => player2.energy >= card.cost);
    
    for (const card of playableCards) {
      // 2. If the card has no target (e.g., "Draw 2 cards"), it's an easy choice.
      if (!card.targeting) {
        resolveCardPlay(card, null, 'player2');
        return; // AI plays the card and ends its action.
      }

      // 3. If the card needs a target, find all valid targets.
      const potentialTargets = getValidTargets('player2', null, card);

      if (potentialTargets.length > 0) {
        // 4. Simple AI: Just pick the first valid target.
        // (This can be made much smarter in the future).
        const target = potentialTargets[0];
        resolveCardPlay(card, target, 'player2');
        return; // AI plays the card and ends its action.
      }
    }
    // --- END NEW --- (If no card was played, proceed to drone attack logic)
