...
const handleFinishOptionalDiscard = () => {
  ...
  }, [passInfo, turn, endTurn, updateAuras, endDeploymentPhase]);
...
// If the AI has already passed, it cannot take any more actions. End the turn immediately.
    ...
    for (const card of playableCards) {
      if (card.targeting) {
        ...
        // *** CHANGE: Add a filter to ensure ship sections are damaged before healing them.
    ...
    for (const attacker of readyAiDrones) {
      ...
      if (sectionName && player1.shipSections[sectionName].hull > 0) {
        ...
        // Only add the section as a target if there is no Guardian drone
        if (!hasGuardian) {
    ...
    possibleActions.forEach(action => {
      ...
      switch (action.type) {
        ...
        case 'play_card':
          ...
          // *** CHANGE: Add very high scoring for healing its own ship sections.
          else if (card.effect.type === 'HEAL_HULL' && card.targeting.type === 'SHIP_SECTION') {
            // This is a top-priority defensive move to prevent losing.
            score = 80;
          }
          action.score = score;
          break;
  ...
  }, [player1, player2, passInfo, placedSections, endActionPhase, endTurn, calculateEffectiveStats]);

    useEffect(() => {
    // We add !aiCardPlayReport here to pause the AI while its card play is being shown.
    const isAiTurn = currentPlayer === 'player2' && !winner && !aiActionReport && !aiCardPlayReport && !pendingAttack && !playerInterceptionChoice && !mandatoryAction && !showFirstPlayerModal && !showActionPhaseStartModal && !showRoundEndModal;
    if (!isAiTurn) return;
        ...
        if (targetType === 'drone') {
          [targetLane] = Object.entries(player2.dronesOnBoard).find(([_, drones]) => drones.some(d => d.id === target.id)) || [];
            ...
            if (laneIndex !== -1) {
                ...
                if (hasGuardian) {
                    setModalContent({ title: "Invalid Target", text: "This ship section is protected by a Guardian drone and cannot be targeted." });
