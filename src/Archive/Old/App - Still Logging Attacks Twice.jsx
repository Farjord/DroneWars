...
// Piercing Attack Card
    ...
    description: 'Deal 2 piercing damage to any drone. This damage ignores shields.',
    targeting: {
      type: 'DRONE',
      affinity: 'ANY', 
      location: 'ANY_LANE'
...
const initialPlayerState = (name) => {
  ...
  return {
    ...
    deploymentBudget: 0,
    hand: [],
    deck: buildDeckFromList(startingDecklist),
    discardPile: [],
    activeDronePool: [],
...
[]);
   ...
   setGameLog(prevLog => [...prevLog, { round: turn, ...entry }]);
...
const resolveAttack = useCallback((attackDetails, isAbilityOrCard = false) => {
    ...
    // --- NEW: Check for the damage type from either the card/ability or the drone itself ---
    ...
    defenderUpdater(prev => {
      ...
      if (finalTargetType === 'drone') {
        ...
        for (const lane in newDronesOnBoard) {
          ...
          if (targetIndex !== -1) {
            ...
            if (damageType !== 'PIERCING') {
              shieldDamage = Math.min(damage, originalTarget.currentShields);
              remainingDamage = damage - shieldDamage;
            }
            hullDamage = Math.min(remainingDamage, originalTarget.hull);
        ...
        // --- MODIFIED: Damage calculation logic for ship sections ---
        let shieldDamage = 0;
        let hullDamage = 0;
        let remainingDamage = damage;

        if (damageType !== 'PIERCING') {
          shieldDamage = Math.min(damage, originalTarget.allocatedShields);
          remainingDamage = damage - shieldDamage;
        }
        hullDamage = Math.min(remainingDamage, originalTarget.hull);
        ...
        if (outcome) {
         addLogEntry({
        player: attackingPlayer === 'player1' ? player1.name : player2.name,
        actionType: 'ATTACK',
    ...
    if (!isAbilityOrCard) {
      ...
      // 2. If there was an interceptor, update its state separately
      if (interceptor) {
        const interceptorUpdater = attackingPlayer === 'player1' ? setPlayer2 : setPlayer1;
        interceptorUpdater(prevInterceptorPlayer => {
          const newInterceptorPlayerState = { ...prevInterceptorPlayer };
          ...
          for (const lane in newInterceptorDronesOnBoard) {
            ...
            if (interceptorIndex !== -1) {
              const interceptorData = newInterceptorDronesOnBoard[lane][interceptorIndex];
              const effectiveStats = calculateEffectiveStats(interceptorData, lane, newInterceptorPlayerState);
      ...
      // 3. Perform other actions after state updates
      setSelectedDrone(null);
  ...
  }, [endTurn, updateAuras, calculateEffectiveStats]);
...
const resolveCardPlay = useCallback((card, target, actingPlayerId) => {
    ...
    if (effect.type === 'DAMAGE') outcome = `Dealt ${effect.value} damage to ${targetName}.`;
    ...
    if (actingPlayerId === 'player2') {
        ...
        if (target) {
            if (target.name) { // Drones have a .name property
                targetDisplayName = target.name;
            } else if (target.id.startsWith('lane')) { // Lanes
    ...
    // 1) Apply immediate changes (pay cost, move card to discard, apply effects that affect the acting player)
...
// 2) Handle effects that target the other player or need additional actions
    if (effect.type === 'DAMAGE') {
      // --- NEW: Handle filtered, multi-target damage ---
      if (effect.scope === 'FILTERED' && target.id.startsWith('lane') && effect.filter) {
        const laneId = target.id;
        // Determine which player's drones to target
        const targetPlayerState = (target.owner === 'player1') 
            ? (actingPlayerId === 'player1' ? player1 : player2) 
        ...
        dronesInLane.forEach(droneInLane => {
            ...
            if (meetsCondition) {
                resolveAttack({
                    ...
                    abilityDamage: effect.value,
                    lane: laneId,
                    goAgain: false, // Multi-target attacks shouldn't grant extra turns
                    damageType: effect.damageType,
                }, true);
            }
        });
      }
      // --- Existing logic for single-target damage ---
      else {
    ...
    // --- NEW, MODIFIED DESTROY LOGIC for the OPPONENT ---
    else if (effect.type === 'DESTROY') {
      opponentUpdater(prev => {
        ...
        if (effect.scope === 'LANE' && target.id) {
          ...
          newState.dronesOnBoard[laneId].forEach(drone => {
            ...
            // Add the new LTE condition here as well
      ...
      // 3) Cleanup and end turn for the acting player
    if (actingPlayerId === 'player1') {
        cancelCardSelection();
        setCardConfirmation(null);
    }
    
if (!effect.goAgain) { // <--- THIS IS THE MODIFIED LINE
  // End the turn only for the human player here.
  // The AI's turn end is handled by the report modal's close button.
  if (actingPlayerId === 'player1') {
    endTurn('player1');
  }
}
...
const handleFinishOptionalDiscard = () => {
  ...
  useEffect(() => {
    ...
    if (checkWinCondition(player2)) {
     setWinner('Player 1');
        addLogEntry({ player: 'SYSTEM', actionType: 'GAME_END', source: 'N/A', target: 'N/A', outcome: 'Player 1 wins!' });
    // Check if Player 2 has met the win condition against Player 1
    } else if (checkWinCondition(player1)) {
     setWinner('Player 2');
        addLogEntry({ player: 'SYSTEM', actionType: 'GAME_END', source: 'N/A', target: 'N/A', outcome: `${player2.name} wins!` });
    }
...
const handleOpponentTurn = useCallback(() => {
    // Exit early if the AI has already passed this phase.
    if (passInfo.player2Passed) {
      endTurn('player2');
      return;
    }

    // --- Step 1: Decide action based on current state (using player2 from scope) ---
    ...
    const deployableDrones = player2.activeDronePool.filter(drone => {
      ...
      const canAfford = turn === 1
        ? cost <= (player2.initialDeploymentBudget + player2.energy)
        : cost <= (player2.deploymentBudget + player2.energy); // Use the new budget
      return canAfford &&
             (player2.deployedDroneCounts[drone.name] || 0) < drone.limit &&
             totalDrones < effectiveStats.cpuLimit;
    }).sort((a, b) => a.class - b.class);

    const availableLanes = ['lane1', 'lane2', 'lane3'].filter(lane => player2.dronesOnBoard[lane].length < 3);
    const canDeploy = deployableDrones.length > 0 && availableLanes.length > 0;
    const willPass = !canDeploy;

    // --- Step 2: Execute the decided action by calling state setters ---
    if (willPass) {
          addLogEntry({ player: player2.name, actionType: 'PASS', source: 'N/A', target: 'N/A', outcome: `Passed during deployment phase.` });
      setPassInfo(prev => {
        ...
        if (newPassInfo.player1Passed) {
          endDeploymentPhase(); // This is ok, it ends the whole phase
        } else {
          endTurn('player2'); // End turn now that state is set
        }
        return newPassInfo;
      });
        } else {
      // Get all the info needed for the deployment
      ...
      addLogEntry({ player: player2.name, actionType: 'DEPLOY', source: droneToDeploy.name, target: targetLane, outcome: `Deployed to ${targetLane}.` });

      // Determine budget and energy cost based on the turn
      let budgetCost = 0;
      if (turn === 1) {
      ...
      // --- Step 3: End the turn AFTER queueing the state update ---
      endTurn('player2');
    }
  }, [passInfo, turn, endTurn, updateAuras, endDeploymentPhase, player2]);
...
const handleStartNewRound = () => {
    setShowRoundEndModal(false);
      addLogEntry({ player: 'SYSTEM', actionType: 'NEW_ROUND', source: `Round ${turn + 1}`, target: 'N/A', outcome: 'New round begins.' });
    ...
    setPlayer1(prev => readyDronesAndRestoreShields({
      ...
      deploymentBudget: p1Stats.deploymentBudget
    }));
...
// If the AI has already passed, it cannot take any more actions. End the turn immediately.
    ...
    for (const attacker of readyAiDrones) {
      ...
      if (sectionName && player1.shipSections[sectionName].hull > 0) {
        ...
        const hasGuardian = playerDronesInLaneForGuard.some(drone => {
            const effectiveStats = calculateEffectiveStats(drone, attacker.lane, player1);
    ...
    for (const drone of readyAiDrones) {
      ...
      [fromLaneIndex - 1, fromLaneIndex + 1].forEach(toLaneIndex => {
        if (toLaneIndex >= 1 && toLaneIndex <= 3) {
          ...
          if (player2.dronesOnBoard[toLane].length < 3) {
    ...
    possibleActions.forEach(action => {
      ...
      switch (action.type) {
          ...
          if (card.effect.type === 'DESTROY') {
            ...
            if (card.effect.scope === 'FILTERED' && target.id.startsWith('lane') && card.effect.filter) {
                ...
                dronesInLane.forEach(drone => {
                    let meetsCondition = false;
                    if (comparison === 'GTE' && drone[stat] >= value) meetsCondition = true;
                    if (comparison === 'LTE' && drone[stat] <= value) meetsCondition = true; // Added LTE check
                    if (meetsCondition) {
                        potentialDamage += card.effect.value;
                        if (card.effect.damageType === 'PIERCING') {
                            potentialDamage += (drone.currentShields || 0); // Add bonus for piercing shields
                        }
                    }
                });
                score = potentialDamage * 10 - (card.cost * 4);
            } else { // Existing single-target damage logic
                score = card.effect.value * 10;
                if (card.effect.damageType === 'PIERCING') {
                    score += (target.currentShields || 0) * 8;
                }
            }
          }
            ...
            // The AI will now only prioritize drawing cards if it has energy left to potentially use them.
            if (energyAfterPlay > 0) {
              // Give a higher score if it has more energy left.
              score = 10 + (energyAfterPlay * 2);
            } else {
              // If it has no energy left, this is a very low-priority move.
              score = 1;
            }
        ...
        case 'attack': {
          ...
          if (targetType === 'drone') {
            score = (attackTarget.class * 10);
            if (attacker.class < attackTarget.class) { score += 20; }
            if (!attackTarget.isExhausted) { score += 10; }
            // --- NEW LOGIC: Add bonus for piercing drone attacks ---
            if (attacker.damageType === 'PIERCING') {
              score += attackTarget.currentShields * 8;
            }
            // --- END NEW LOGIC ---
          } else if (targetType === 'section') {
            score = (attacker.attack * 8);
            const status = getShipStatus(attackTarget);
            if (status === 'damaged') { score += 15; }
            if (status === 'critical') { score += 30; }
            if (attackTarget.allocatedShields === 0) {
                score += 40;
            }
            else if (attacker.attack >= attackTarget.allocatedShields) {
                score += 35;
            }
            if (attacker.attack >= 3) {
                score += 10;
            }
            // --- NEW LOGIC: Add bonus for piercing attacks on ship sections ---
            if (attacker.damageType === 'PIERCING') {
              score += attackTarget.allocatedShields * 10;
            }
            // --- END NEW LOGIC ---
          }
          action.score = score;
        ...
        case 'move': {
          ...
          if (isSectionDestroyed) {
            ...
            if (!isFasterThanAllEnemies) { score = 5; }
          }
          action.score = score;
          break;
        }
    ...
    if (topScore <= 0) {
          addLogEntry({ player: player2.name, actionType: 'PASS', source: 'N/A', target: 'N/A', outcome: `Passed during action phase.` });
    ...
    switch (chosenAction.type) {
      case 'play_card':
        resolveCardPlay(chosenAction.card, chosenAction.target, 'player2');
        break;
      case 'attack':
        setPendingAttack({
      ...
      case 'move':
        const { drone, fromLane, toLane } = chosenAction;
            ...
            addLogEntry({ player: player2.name, actionType: 'MOVE', source: drone.name, target: toLane, outcome: `Moved from ${fromLane} to ${toLane}.` });
 ...
 const handleSelectOpponent = (selectedAI) => {
    ...
    setPlayer2(prev => {
      ...
      // Build the AI's deck using its specific decklist
      const aiDeck = buildDeckFromList(selectedAI.decklist);

      // Return a new state for Player 2, keeping the base state but overriding the key parts
      return {
        ...initialPlayerState(selectedAI.name), // Use AI's name
...
const handleConfirmDroneSelection = (selectedDrones) => {
  ...
  const handlePlayerPass = () => {
    if (passInfo.player1Passed) return;
     addLogEntry({ player: player1.name, actionType: 'PASS', source: 'N/A', target: 'N/A', outcome: `Passed during ${turnPhase} phase.` });
   setPassInfo(prev => {
...
useEffect(() => {
    ...
    if (attackingPlayer === 'player1') {
      // --- CHANGE: Use player2Ref.current instead of player2 ---
      const potentialInterceptors = player2Ref.current.dronesOnBoard[lane]
        .filter(d => {
            // --- CHANGE: Use player2Ref.current here as well ---
            const effectiveInterceptor = calculateEffectiveStats(d, lane, player2Ref.current);
      ...
      // --- CHANGE: Use player1Ref.current instead of player1 ---
      const potentialInterceptors = player1Ref.current.dronesOnBoard[lane]
        .filter(d => {
            // --- CHANGE: Use player1Ref.current here as well ---
            const effectiveInterceptor = calculateEffectiveStats(d, lane, player1Ref.current);
            return !d.isExhausted &&
                   (effectiveInterceptor.speed > attacker.speed || effectiveInterceptor.keywords.has('ALWAYS_INTERCEPTS')) &&
                   (targetType !== 'drone' || d.id !== target.id)
        });
...
const handleLaneClick = (e, lane, isPlayer) => {
 ...
 if (!card.targeting) {
  ...
  const executeMove = () => {
    ...
    addLogEntry({ player: player1.name, actionType: 'MOVE', source: drone.name, target: to, outcome: `Moved from ${from} to ${to}.` });
  ...
  const getPhaseDisplayName = (phase) => {
    ...
    const downloadLogAsCSV = () => {
      ...
      const headers = ['Round', 'Player', 'Action', 'Source', 'Target', 'Outcome'];
