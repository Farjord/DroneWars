...
[]);
  ...
  const addLogEntry = useCallback((entry, debugSource) => {
   // We use the 'turn' state variable directly here
 setGameLog(prevLog => [...prevLog, { round: turn, debugSource, ...entry }]);
...
const resolveAttack = useCallback((attackDetails, isAbilityOrCard = false) => {
    ...
    const defenderStateSetter = (target.owner && target.owner === 'player1') ? setPlayer1 : (attackingPlayer === 'player1' ? setPlayer2 : setPlayer1);
    const attackerStateSetter = attackingPlayer === 'player1' ? setPlayer1 : setPlayer2;

    // --- REFACTOR START ---
    // We get the current state using a ref to calculate the outcome *before* updating the state.
    const defenderPlayerState = (target.owner && target.owner === 'player1') ? player1Ref.current : (attackingPlayer === 'player1' ? player2Ref.current : player1Ref.current);
    ...
    // Perform the side-effects (logging, AI reports) ONCE.
    addLogEntry({
        player: attackingPlayer === 'player1' ? player1Ref.current.name : player2Ref.current.name,
        actionType: 'ATTACK',
            ...
            interceptorUpdater(prev => {
                ...
                for (const lane in newDronesOnBoard) {
                    ...
                    if (interceptorIndex !== -1) {
                        const effectiveStats = calculateEffectiveStats(newDronesOnBoard[lane][interceptorIndex], lane, prev);
...
const handleOpponentTurn = useCallback(() => {
    ...
    if (willPass) {
          addLogEntry({ player: player2.name, actionType: 'PASS', source: 'N/A', target: 'N/A', outcome: `Passed during deployment phase.` }, 'aiDeploymentPass');
      setPassInfo(prev => {
      ...
      addLogEntry({ player: player2.name, actionType: 'DEPLOY', source: droneToDeploy.name, target: targetLane, outcome: `Deployed to ${targetLane}.` }, 'aiDeploymentDeploy');
...
// If the AI has already passed, it cannot take any more actions. End the turn immediately.
    ...
    if (topScore <= 0) {
          addLogEntry({ player: player2.name, actionType: 'PASS', source: 'N/A', target: 'N/A', outcome: `Passed during action phase.` }, 'aiActionPass');
    ...
    switch (chosenAction.type) {
      ...
      case 'move':
        const { drone, fromLane, toLane } = chosenAction;
            ...
            addLogEntry({ player: player2.name, actionType: 'MOVE', source: drone.name, target: toLane, outcome: `Moved from ${fromLane} to ${toLane}.` }, 'aiActionMove');
