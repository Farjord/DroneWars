...
const startingDecklist = [
    ...
    { id: 'CARD006', quantity: 10 },
    { id: 'CARD007', quantity: 10 },
    { id: 'CARD008', quantity: 10 },
];
...
// --- MODIFIED --- Now handles cards as well as abilities
  const getValidTargets = useCallback((source, definition) => {
    ...
    if (type === 'DRONE') {
      ...
      if (affinity === 'ENEMY' || affinity === 'ANY') {
       ...
       if (affinity === 'FRIENDLY' || affinity === 'ANY') {
         ['lane1', 'lane2', 'lane3'].forEach(laneId => targets.push({ id: laneId, owner: 'player1' }));
       }
       if (affinity === 'ENEMY' || affinity === 'ANY') {
         ['lane1', 'lane2', 'lane3'].forEach(laneId => targets.push({ id: laneId, owner: 'player2' }));
       }
    }
    return targets;
  ...
  useEffect(() => {
    if (abilityMode) {
      ...
      setValidCardTargets(targets.map(t => t.id));
  ...
  const resolveCardPlay = useCallback((card, target) => {
    ...
    // 2. Apply effect - Add other effect types here for future cards...
  ...
  if (targetLane) {
    resolveAttack({
        ...
        setPlayer1(prev => {
            ...
            for (let i = 0; i < effect.value; i++) {
                ...
                if (newDeck.length === 0) {
                    // ...check if there are cards in the discard pile to reshuffle.
                    if (newDiscardPile.length > 0) {
                        newDeck = [...newDiscardPile].sort(() => 0.5 - Math.random());
                        newDiscardPile = []; // The discard pile is now empty
                    } else {
            ...
            // Return the new state, including the potentially emptied discard pile.
            return { ...prev, deck: newDeck, hand: newHand, discardPile: newDiscardPile };
        });
    } 
  else if (effect.type === 'GAIN_ENERGY') {
        setPlayer1(prev => {
            // Calculate current max energy based on ship status
            const effectiveStatsP1 = calculateEffectiveShipStats(prev);
            
            // Don't do anything if energy is already at or above the max
            if (prev.energy >= effectiveStatsP1.maxEnergy) return prev;
            
            // Calculate the new energy, ensuring it doesn't go over the cap
            const newEnergy = Math.min(effectiveStatsP1.maxEnergy, prev.energy + effect.value);
        ...
        setPlayer1(prev => {
            ...
            // Find the target drone by its ID and update its status
            for (const lane in newDronesOnBoard) {
                const droneIndex = newDronesOnBoard[lane].findIndex(d => d.id === target.id);
                if (droneIndex !== -1) {
                    newDronesOnBoard[lane][droneIndex].isExhausted = false;
      ...
      // The owner of the target determines which player state to update
      const targetPlayerUpdater = target.owner === 'player1' ? setPlayer1 : setPlayer2;

      targetPlayerUpdater(prev => {
        // Create a deep copy to avoid direct state mutation
        const newPlayerState = JSON.parse(JSON.stringify(prev));
        const healValue = effect.value;

        // Logic for healing a single DRONE
        if (card.targeting.type === 'DRONE') {
          for (const lane in newPlayerState.dronesOnBoard) {
            const droneIndex = newPlayerState.dronesOnBoard[lane].findIndex(d => d.id === target.id);
            if (droneIndex !== -1) {
              const droneToHeal = newPlayerState.dronesOnBoard[lane][droneIndex];
              if (isHull) {
                const baseDrone = fullDroneCollection.find(d => d.name === droneToHeal.name);
                droneToHeal.hull = Math.min(baseDrone.hull, droneToHeal.hull + healValue);
              } else { // isShields
                droneToHeal.currentShields = Math.min(droneToHeal.currentMaxShields, droneToHeal.currentShields + healValue);
              }
              break;
            }
          }
        } 
        // Logic for healing a single SHIP SECTION
        else if (card.targeting.type === 'SHIP_SECTION') {
          const sectionToHeal = newPlayerState.shipSections[target.id];
          if (sectionToHeal) {
            if (isHull) {
              sectionToHeal.hull = Math.min(sectionToHeal.maxHull, sectionToHeal.hull + healValue);
            } else { // isShields
              sectionToHeal.allocatedShields = Math.min(sectionToHeal.shields, sectionToHeal.allocatedShields + healValue);
            }
          }
        }
        // Logic for healing all drones in a LANE
        else if (card.targeting.type === 'LANE') {
          const laneToHeal = newPlayerState.dronesOnBoard[target.id];
          if(laneToHeal) {
            laneToHeal.forEach(droneToHeal => {
               if (isHull) {
                const baseDrone = fullDroneCollection.find(d => d.name === droneToHeal.name);
                droneToHeal.hull = Math.min(baseDrone.hull, droneToHeal.hull + healValue);
              } else { // isShields
    ...
    // 4. End turn (if the effect wasn't damage, which already ends the turn via resolveAttack)
    if (effect.type !== 'DAMAGE' && !effect.goAgain) { 
      endTurn('player1');
    }
  }, [player1, player2, getLaneOfDrone, resolveAttack, endTurn]);
  ...
  const handleTargetClick = (target, targetType, isPlayer) => {
    ...
    if (selectedCard && validCardTargets.includes(target.id)) {
      setCardConfirmation({ card: selectedCard, target });
      return;
    }
  ...
  const handleTokenClick = (e, token, isPlayer) => {
      ...
      if ((abilityMode && validAbilityTargets.includes(token.id)) || (selectedCard && validCardTargets.includes(token.id))) {
    ...
    if (abilityMode) {
     cancelAbilityMode();
      return;
        ...
        // Check if the clicked lane is a valid target for the selected card
      ...
      <div className="flex flex-wrap gap-2 p-1 min-h-[100px] justify-center items-center">
       {drones.map((drone) => {
            ...
            return (
                ...
                isActionTarget={validAbilityTargets.includes(drone.id) || validCardTargets.includes(drone.id)}
                        ...
                        {sectionName ? (
                            <ShipSection
                                ...
                                isInteractive={isInteractive}
                                  onClick={() => {
                                      // This function now handles all click scenarios for a ship section
                                      const targetData = { name: sectionName, ...player.shipSections[sectionName] };

                                      // Check if the click should be treated as a card target first
                                      if (selectedCard && validCardTargets.includes(sectionName)) {
                                          onTargetClick(targetData, 'section', isPlayer);
                                          return;
                                      }

                                      // If not targeting with a card, check for other interactive states
                                      if (isPlayer) {
                                          if (isInteractive) {
                                              onSectionClick(sectionName); // This handles shield allocation
                                          }
                                      } else {
                                          // This handles opponent targeting (for attacks, etc.)
                                          onTargetClick(targetData, 'section', isPlayer);
                                      }
                                  }}
                                isOpponent={!isPlayer}
            ...
            <>
                {turnPhase === 'placement' ? (
                  ...
                  <div className="flex flex-col items-center w-full space-y-2">
                      ...
                      <ShipSectionsDisplay player={player1} isPlayer={true} placedSections={placedSections} onSectionClick={handleAllocateShield} onTargetClick={handleTargetClick} isInteractive={turnPhase === 'allocateShields'} />
                  </div>
                )}
            </>
        )}
               ...
               <div className="flex flex-col items-center">
                 ...
                 <div className="flex flex-wrap justify-center gap-4">{player1.hand.map((card) => 
                   <ActionCard 
                     ...
                     onClick={mandatoryAction?.type === 'discard' 
 ...
 {confirmationModal && (
  ...
  {cardConfirmation && (
    <GamePhaseModal
      ...
      text={`Use ${cardConfirmation.card.name}${cardConfirmation.target ? ` on ${cardConfirmation.target.name}` : ''}? This will cost ${cardConfirmation.card.cost} energy.`}
