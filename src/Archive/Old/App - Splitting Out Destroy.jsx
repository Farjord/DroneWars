...
// --- NEW: LOGIC TO RESOLVE A CARD ---
  const resolveCardPlay = useCallback((card, target) => {
    ...
    setPlayer1(prev => {
        ...
        // DESTROY (this clears the named lane for player1; player2 handled below too)
        else if (effect.type === 'DESTROY') {
            if (target && target.id) {
                const laneId = target.id;
                const newDronesOnBoard = JSON.parse(JSON.stringify(newState.dronesOnBoard));
                const destroyed = newDronesOnBoard[laneId] || [];
                // Update deployed counts using helper
                destroyed.forEach(d => {
                    triggerExplosion(d.id); // ADD THIS LINE
                    const updates = onDroneDestroyed(newState, d);
                    if (updates.deployedDroneCounts) {
    ...
    // If the card destroys a lane, make sure we clear the opponent's lane too
    else if (effect.type === 'DESTROY') {
        if (target && target.id) {
            const laneId = target.id;
            setPlayer2(prev => {
            ...
            // Also refresh player1 auras (in case DESTROY removed buffs)
            setPlayer1(prev => {
                const newState = JSON.parse(JSON.stringify(prev));
                newState.dronesOnBoard = updateAuras(newState);
                return newState;
    ...
    // (Optional) additional opponent-targeted heal handlers left intentionally (not present currently)
