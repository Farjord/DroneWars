...
const createStartingDeck = () => {
  const deck = [];
  for (let i = 0; i < 40; i++) {
    deck.push({ id: `action-${i}`, type: 'Action', name: `Action Card ${i + 1}`, cost: Math.floor(Math.random() * 5) + 1 });
  }
  return deck;
};
...
const initialPlayerState = (name) => {
  const shipSections = {
    bridge: {
      ...
      thresholds: { damaged: 7, critical: 4 },
      stats: {
    ...
    powerCell: {
      ...
      stats: {
        ...
        damaged: { 'Energy Per Turn': 7, 'Max Energy': 8, 'Shields Per Turn': 3 },
        critical: { 'Energy Per Turn': 5, 'Max Energy': 6, 'Shields Per Turn': 2 },
      }
    },
    droneControlHub: {
      hull: 10, maxHull: 10, shields: 3, allocatedShields: 3,
      ...
      stats: {
        ...
        damaged: { 'Initial Deployment': 5, 'CPU Control Value': 7 },
        critical: { 'Initial Deployment': 4, 'CPU Control Value': 5 },
      }
    }
  };
...
const App = () => {
  ...
  const resolveAttack = useCallback((attackDetails, isAbility = false) => {
   ...
   defenderUpdater(prev => {
        ...
        if (finalTargetType === 'drone') {
            ...
            // NEW: After taking damage, cap energy if necessary
            let newEnergy = newPlayerState.energy;
            const newEffectiveStats = calculateEffectiveShipStats({ ...newPlayerState, shipSections: {...newPlayerState.shipSections, [finalTarget.name]: {...originalTarget, hull:newHull} } });
           ...
           newShipSections[finalTarget.name] = { ...originalTarget, allocatedShields: newShields, hull: newHull };
            ...
            newPlayerState.energy = newEnergy; // Apply capped energy
        }
  ...
  const resolveAbility = useCallback((ability, userDrone, targetDrone) => {
    ...
    const effectiveStatsP1 = calculateEffectiveShipStats(player1);
    ...
    if (effect.type === 'HEAL') {
        ...
        // Example of how to handle new effects safely
        setPlayer1(prev => {
  ...
  }, [endTurn, player1, player2, getLaneOfDrone, resolveAttack]); 

  //--- END ABILITY LOGIC ---

  const handleFooterButtonClick = (view) => {
    if (!isFooterOpen) {
      // If closed, open it and set to the clicked view
      setIsFooterOpen(true);
      setFooterView(view);
    } else {
      // If already open...
      if (footerView === view) {
        // ...and this is the active view, close it
        setIsFooterOpen(false);
      } else {
        // ...and this is NOT the active view, just switch to it
        setFooterView(view);
      }
    }
  };
  ...
  const handleStartNewRound = () => {
    ...
    // Calculate new stats BEFORE applying them
    ...
    // Start of new compliance checks
    if (player1.hand.length > p1Stats.handLimit) {
        ...
        // If player 1 is fine, check player 2 (AI)
        // AI logic for mandatory discard
        ...
        setTurnPhase('initialDraw'); // Proceed if no action needed for P1
    }
    ...
    const resolvePlayerCompliance = (player) => {
      if (player === 'player1') {
        ...
        if (secondPlayerIsOverLimit) {
            // AI Drone Destruction Logic
            setPlayer2(p2 => {
                let newP2 = {...p2};
                let dronesToDestroyCount = totalPlayer2Drones - player2EffectiveStats.cpuLimit;
      ...
      return false; // No action required for this player
    };
    
    // Start checking in player order
    if (!resolvePlayerCompliance(checkOrder[0])) {
      // If first player is fine, check second
      if (!resolvePlayerCompliance(checkOrder[1])) {
        // If both are fine, proceed
  ...
  }, [currentPlayer, turnPhase, playerPassState, winner, handleOpponentTurn, handleOpponentAction, aiActionReport, pendingAttack, playerInterceptionChoice, mandatoryAction]);
  ...
  const handleTokenClick = (e, token, isPlayer) => {
      // New: Check for mandatory destroy action
  ...
  const handleConfirmMandatoryDiscard = (card) => {
    ...
    setMandatoryAction(prev => {
        ...
        if (newCount <= 0) {
            handlePostDiscardAction(); // Check AI and proceed
            return null;
        }
  ...
  const DroneToken = ({ drone, onClick, isPlayer, isSelected, isHit, isPotentialInterceptor, onMouseEnter, onMouseLeave, effectiveStats, onAbilityClick, isAbilityTarget }) => {
    ...
    return (
      <div
        ...
        className={`relative w-24 bg-gray-900/80 rounded-lg flex flex-col p-2 shadow-lg border-2 ${borderColor} cursor-pointer transition-transform duration-200 hover:scale-110 ${exhaustEffect} ${selectedEffect} ${hitEffect} ${interceptorEffect} ${abilityTargetEffect} ${mandatoryDestroyEffect} shadow-black`}
      >
    ...
    return (
      ...
      >
        ...
        >
          ...
          {/* Image */}
          <div className="p-1 flex-shrink-0 h-14"> {/* Increased height for image */}
          ...
          {/* Stats */}
           <div className="flex flex-col gap-1 px-2 flex-shrink-0 h-[50px] mt-2"> {/* Added margin top */}
  ...
  const ShipSection = ({ section, stats, isPlayer, isPlaceholder, onClick, isInteractive, isOpponent, isHovered, onMouseEnter, onMouseLeave }) => {
    ...
    const bgColor = status === 'critical' ? 'bg-red-900/60' : status === 'damaged' ? 'bg-yellow-900/40' : (isOpponent ? 'bg-black/30' : 'bg-black/30');
  ...
  const ShipSectionsDisplay = ({ player, isPlayer, placedSections, onSectionClick, isInteractive, onTargetClick }) => {
    return (
  ...
  const CardInHand = ({ card, onClick, isSelectable }) => {
    const IconComponent = iconMap[card.type === 'Action' ? 'Sprout' : 'Wrench'];
    return (
      <div onClick={isSelectable ? () => onClick(card) : undefined}
           className={`bg-gray-900/80 backdrop-blur-sm text-white rounded-lg p-3 w-32 flex flex-col items-center justify-between shadow-lg border border-gray-600 transition-all duration-200 
           ${isSelectable ? 'cursor-pointer hover:border-red-500 hover:shadow-red-500/30 ring-2 ring-red-500 animate-pulse' : 'hover:border-purple-500 hover:shadow-purple-500/30'}`}>
  ...
  return (
    <div className="h-screen bg-gray-950 text-white font-sans overflow-hidden flex flex-col bg-gradient-to-br from-gray-900 via-indigo-950 to-black relative" ref={gameAreaRef} onClick={cancelAbilityMode}>
      ...
      <header className="w-full flex justify-between items-center mb-2 flex-shrink-0 px-5 pt-8">
          {turnPhase !== 'preGame' && (
              <div className="flex flex-col items-start gap-2">
                  ...
                  {(turnPhase === 'deployment' || turnPhase === 'action') && playerPassState.player2 && <span className="text-base font-semibold text-red-400 ml-2">(Passed)</span>}
                  </h2>
          ...
          {turnPhase !== 'preGame' && (
              <div className="flex flex-col items-end gap-2">
                  ...
                  {(turnPhase === 'deployment' || turnPhase === 'action') && playerPassState.player1 && <span className="text-base font-semibold text-red-400 ml-2">(Passed)</span>}
                  </h2>
            ...
            <>
                ...
                <DroneSelectionScreen 
                  ...
                  <div className="flex flex-col items-center w-full space-y-2">
                    <ShipSectionsDisplay player={player2} isPlayer={false} placedSections={opponentPlacedSections} onTargetClick={handleTargetClick} isInteractive={false} />
                    ...
                    <ShipSectionsDisplay player={player1} isPlayer={true} placedSections={placedSections} onSectionClick={handleAllocateShield} isInteractive={turnPhase === 'allocateShields'} />
                  </div>
                )}
            </>
        )}
          ...
          <div className={`relative w-full bg-slate-800/80 backdrop-blur-sm transition-all duration-500 ease-in-out overflow-hidden ${isFooterOpen ? 'max-h-[500px] opacity-100 p-4' : 'max-h-0 opacity-0'}`}>
              {(turnPhase === 'deployment' || turnPhase === 'action') && currentPlayer === 'player1' && !mandatoryAction && (
                  <button
                      ...
                      disabled={playerPassState.player1}
                      className={`absolute top-4 right-4 z-10 text-white font-bold py-2 px-4 rounded-full transition-colors duration-200 ${
                          playerPassState.player1 ? 'bg-gray-700 cursor-not-allowed' : 'bg-red-600 hover:bg-red-500'
                      }`}
                  >
                      Pass
                  </button>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <div className="flex flex-wrap justify-center gap-4">{player1.hand.map((card) => 
                    <CardInHand 
                        key={card.id} 
                        card={card} 
                        isSelectable={mandatoryAction?.type === 'discard'}
                        onClick={(c) => setConfirmationModal({type: 'discard', target: c, onConfirm: () => handleConfirmMandatoryDiscard(c), onCancel: () => setConfirmationModal(null), text: `Are you sure you want to discard ${c.name}?`})}
                    />)}
                  </div>
                </div>
                <div className="flex flex-col items-center w-32 min-w-32">
              ...
              <div className="flex flex-col items-center mb-4 w-full">
                <div className="flex flex-wrap justify-center gap-4">
                    {sortedPlayer1ActivePool.map((drone, index) => {
                        ...
                        return <DroneCard key={index} drone={drone} onClick={handleToggleDroneSelection} isSelected={selectedDrone && selectedDrone.name === drone.name} isSelectable={turnPhase === 'deployment' && currentPlayer === 'player1' && !playerPassState.player1 && canAfford && !mandatoryAction} deployedCount={player1.deployedDroneCounts[drone.name] || 0} />;
                    })}
                </div>
              </div>
            )}
      ...
      {showFirstPlayerModal && (
         <GamePhaseModal title="First Player Determined" text={`The first player is determined randomly. ${currentPlayer === 'player1' ? player1.name : player2.name} will go first this round.`} onClose={startDeploymentComplianceCheck}>
