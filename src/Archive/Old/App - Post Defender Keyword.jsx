...
{ 
    name: 'Interceptor', 
    class: 3, limit: 3, attack: 1, hull: 2, shields: 3, speed: 2, 
...
const resolveCardPlay = useCallback((card, target, actingPlayerId) => {
    ...
    if (effect.type === 'DAMAGE') {
        // Determine which player state to search for the target drone
        ...
        if (targetLane) {
resolveAttack({
            attacker: { name: card.name }, // The card itself is the source
            target: target,
            targetType: 'drone',
            attackingPlayer: actingPlayerId, // The player who played the card is the attacker
            abilityDamage: effect.value,
            lane: targetLane,
...
const handleOpponentTurn = useCallback(() => {
    ...
    if (willPass) {
      ...
      const budgetCost = turn === 1 ? Math.min(player2.initialDeploymentBudget, droneCost) : 0;
    ...
    possibleActions.forEach(action => {
      ...
      switch (action.type) {
        ...
        case 'play_card': {
          ...
          // --- NEW LOGIC: Scoring for DAMAGE cards, including piercing ---
