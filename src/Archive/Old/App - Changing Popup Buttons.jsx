...
import { Shield, Bolt, Wrench, Sprout, Hand, ShipWheel, Settings, X, ChevronRight, ChevronLeft, Plus, RotateCcw, ShieldCheck, Sword, Search, Gavel, Bomb, Rocket, Skull, Bug, Cpu, Target, View, Zap, Heart } from 'lucide-react';
...
            w-48 h-[400px] rounded-lg p-[2px] relative group
...
          <div className="text-center py-1 px-3 bg-cyan-900/50 flex-shrink-0 h-10 flex items-center justify-center">

              <ScalingText text={name} className="font-orbitron text-md uppercase tracking-widest whitespace-nowrap" />
...
          <div className="p-2 flex-shrink-0 h-28">

              <div className="relative h-full" style={{ clipPath: 'polygon(20px 0, 100% 0, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0 100%, 0 20px)' }}>
...
           <div className="flex flex-col gap-2 px-3 flex-shrink-0 h-[70px]">

              <div className="flex justify-between gap-2 text-sm">

                  <div className="flex items-center justify-center gap-2 bg-cyan-900/50 p-1 rounded w-1/2"><Sword size={16} /> {attack}</div>

                  <div className="flex items-center justify-center gap-2 bg-cyan-900/50 p-1 rounded w-1/2"><Rocket size={16} /> {speed}</div>

              </div>

              <div className="flex flex-col items-center gap-1 mt-1">

                  <div className="flex w-full justify-center gap-1.5 min-h-[16px]">

                      {shields > 0 && Array.from({ length: shields }).map((_, i) => (

                          <ShieldCheck key={`shield-${i}`} size={16} className="text-cyan-300" />
...
                          <div key={`hull-${i}`} className="h-4 w-3 rounded-sm bg-green-500 border border-green-300/50"></div>
...
          <div className="flex-grow mx-2 mt-2 bg-black/50 border border-cyan-800/70 p-2 flex flex-col min-h-0">

              {ability ? (

                  <>

                      <h4 className="text-sm text-purple-400 tracking-wider flex-shrink-0">{ability.name}</h4>

                      <div className="flex-grow relative font-exo font-normal text-cyan-200">

                         <ScalingText text={ability.description} className="text-gray-400 leading-tight" />
...
                   <p className="text-sm text-cyan-700 italic opacity-70">[ No Signal ]</p>
...
           <div className="flex items-center justify-between p-2 border-t border-cyan-800/70 flex-shrink-0 h-16">
...
                        <Bolt size={16} className="text-yellow-300"/>

                        <span className="text-white font-bold text-lg ml-1">{deploymentCost}</span>
...
                    <span className="text-xs text-gray-400 mb-1">Deployed</span>

                    <div className="flex items-center">

                        <span className={`font-bold text-lg ml-1 ${atLimit ? 'text-pink-500' : 'text-white'}`}>
...
                {showFirstPlayerIndicator && isDesignatedFirstPlayer && <span className="text-lg text-yellow-300 ml-4">(First Player)</span>}

                {(turnPhase === 'deployment' || turnPhase === 'action') && hasPassed && <span className="text-lg text-red-400 ml-4">(Passed)</span>}
...
                             className={`min-h-[160px] h-auto rounded-lg border-2 p-4 transition-all duration-200
...
                            <p className={`text-center mt-2 ${isPlayer ? 'text-cyan-300' : 'text-pink-300'}`}>
...
    <div className="min-h-screen bg-gray-950 text-white font-sans p-8 flex flex-col items-center bg-gradient-to-br from-gray-900 via-indigo-950 to-black relative" ref={gameAreaRef} onClick={cancelAbilityMode}>
...
                  <div key={laneIndex} className="col-span-1" onClick={() => handlePlaceSection(laneIndex)}>
...
              <div className="flex flex-col items-center w-full mb-8"><ShipSectionsDisplay player={player2} isPlayer={false} placedSections={opponentPlacedSections} onTargetClick={handleTargetClick} isInteractive={false} firstPlayerOfRound={firstPlayerOfRound} turnPhase={turnPhase} playerPassState={playerPassState} /></div>

              <div className="flex flex-col items-center w-full mb-8"><DroneLanesDisplay player={player2} isPlayer={false} placedSections={opponentPlacedSections} onLaneClick={() => {}} selectedDrone={selectedDrone} /></div>

              <div className="w-full max-w-6xl h-px bg-gradient-to-r from-transparent via-purple-500 to-transparent my-8"></div>

              <div className="flex flex-col items-center w-full mb-8"><DroneLanesDisplay player={player1} isPlayer={true} placedSections={placedSections} onLaneClick={handleLaneClick} selectedDrone={selectedDrone} /></div>

              <div className="flex flex-col items-center w-full"><ShipSectionsDisplay player={player1} isPlayer={true} placedSections={placedSections} onSectionClick={handleAllocateShield} isInteractive={turnPhase === 'allocateShields'} firstPlayerOfRound={firstPlayerOfRound} turnPhase={turnPhase} playerPassState={playerPassState} /></div>

             <footer className="w-full mt-16 max-w-6xl flex flex-col items-center">
