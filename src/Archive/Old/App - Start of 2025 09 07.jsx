...
const createStartingDeck = () => {
  const deck = [];
  const cardTemplate = fullCardCollection.find(c => c.id === 'CARD001');
  if (cardTemplate) {
    for (let i = 0; i < 30; i++) {
      deck.push(createCard(cardTemplate, `card-${i}`));
    }
  }
  return deck;
};
...
const App = () => {
  ...
  const resolveCardPlay = useCallback((card, target) => {
    ...
    if (effect.type === 'DAMAGE') {
      ...
      if (targetLane) {
        resolveAttack({
          attacker: { name: card.name }, // The card is the "attacker"
          target: target,
          targetType: 'drone',
          attackingPlayer: 'player1',
          abilityDamage: effect.value,
 ...
 {confirmationModal && (
  ...
  {cardConfirmation && (
    <GamePhaseModal
      ...
      text={`Use ${cardConfirmation.card.name} on ${cardConfirmation.target.name}?`}
