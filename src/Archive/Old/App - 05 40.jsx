...
const startingDecklist = [
    { id: 'CARD001', quantity: 20 },
    { id: 'CARD002', quantity: 20 },
    { id: 'CARD003', quantity: 20 },
];
...
const App = () => {
  ...
  const getValidTargets = useCallback((source, definition) => {
    ...
    const processPlayer = (playerState, playerType) => {
    ...
    if (type === 'DRONE') {
      if (affinity === 'FRIENDLY' || affinity === 'ANY') {
        processPlayer(player1, 'player1');
      }
      if (affinity === 'ENEMY' || affinity === 'ANY') {
        processPlayer(player2, 'player2');
      }
  ...
  if (targetLane) {
    resolveAttack({
          ...
          abilityDamage: effect.value,
          lane: targetLane,
          goAgain: effect.goAgain || false, // --- MODIFIED --- Pass the flag
        }, true);
      }
    }
    // Add other effect types here for future cards...
               ...
               <div className="flex flex-col items-center">
                 ...
                 <div className="flex flex-wrap justify-center gap-4">{player1.hand.map((card) => 
                   <ActionCard 
                     key={card.instanceId} 
                     card={card} 
                     isPlayable={turnPhase === 'action' && currentPlayer === 'player1' && !passInfo.player1Passed && player1.energy >= card.cost}
