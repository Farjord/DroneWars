...
const resolveAbility = useCallback((ability, userDrone, targetDrone) => {
    ...
    if (effect.type === 'HEAL') {
        ...
        setPlayer1(prev => {
            ...
            if (prev.energy >= effectiveStatsP1.totals.maxEnergy) return prev;
  ...
  }, [endTurn, player2, getLaneOfDrone, resolveAttack, player1.name, addLogEntry]);
...
if (selectedCard && validCardTargets.some(t => t.id === target.id)) {
  ...
  const handleAbilityIconClick = (e, drone, ability) => {
    ...
    if (ability.targeting?.type === 'LANE' && ability.targeting?.location === 'SAME_LANE') {
        const laneId = getLaneOfDrone(drone.id, player1);
        if (laneId) {
...
const DestroyUpgradeModal = ({ selectionData, onConfirm, onCancel }) => {
  ...
  const ShipSection = ({ section, stats, effectiveStatsForDisplay, isPlayer, isPlaceholder, onClick, isInteractive, isOpponent, isHovered, onMouseEnter, onMouseLeave, isCardTarget, isInMiddleLane }) => {
    ...
    // --- FIX: Define sectionStatus here ---
    const sectionStatus = getShipStatus(stats);
    
    // --- FIX: Use sectionStatus for colors and borders ---
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center p-4 h-full">
            {/* --- FIX: Use sectionStatus for the status tag --- */}
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="flex w-full justify-center gap-1">
                  {Array.from({ length: stats.maxHull }).map((_, i) => {
                      ...
                      return (
                        <div key={i} className={`h-4 w-4 rounded-sm ${isFilled ? thresholdColor : 'bg-black/50'} border border-black/50`}></div>
                      );
                  })}
                </div>
              </div>
