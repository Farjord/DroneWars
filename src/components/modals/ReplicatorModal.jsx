...
const ReplicatorModal = ({ onClose }) => {
  ...
  let REPLICATE_COSTS = { Common: 50, Uncommon: 125, Rare: 300, Mythic: 750 };
  try {
    REPLICATE_COSTS = replicatorService.getAllCosts() || REPLICATE_COSTS;
  } catch (e) {
            ...
            <>
              ...
              <div className="dw-modal-info-box" style={{ marginTop: '16px', background: 'rgba(147, 51, 234, 0.1)', borderColor: 'rgba(147, 51, 234, 0.3)' }}>
                <p style={{ fontSize: '12px', color: 'var(--modal-text-primary)', margin: 0 }}>
                  <strong style={{ color: '#a855f7' }}>Replicate Costs:</strong>{' '}
                  Common {REPLICATE_COSTS.Common}, Uncommon {REPLICATE_COSTS.Uncommon}, Rare {REPLICATE_COSTS.Rare}, Mythic {REPLICATE_COSTS.Mythic} credits
                </p>
              </div>
            </>
          )}
