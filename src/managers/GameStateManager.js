...
class GameStateManager {
  constructor() {
    ...
    this.state = {
      ...
      gameActive: false,
      gameSeed: null, // Random seed for deterministic gameplay (generated by host/local, received by remote client)
      turnPhase: null,
      turn: null,
      currentPlayer: null,
    ...
    // Optimistic animation tracker (dedup non-authority-played animations from host responses)
   ...
   * Remote-client-side P2P subscriptions (state_update_received) are handled by P2PTransport.
   */
  ...
  startGame(gameMode = 'local', player1Config = {}, player2Config = {}, options = {}) {
    ...
    // Host/Local generates, remote client receives seed from host's first broadcast
    // GameServer is created by AppRouter when appState transitions to 'inGame'
    // so gameMode param is the correct signal here.
    const isRemoteClient = gameMode === 'guest';
    const gameSeed = isRemoteClient
      ? null
  ...
  /**
   * Check if this client is the remote (guest/player2) side of a multiplayer game
   */
  isRemoteClient() {
    return this.getLocalPlayerId() === 'player2';
  }
  ...
  getLocalPlacedSections() {
    return this.isRemoteClient() ? this.state.opponentPlacedSections : this.state.placedSections;
  }
